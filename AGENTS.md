...
- `pnpm local` is the same environment pinned to the local relay and Blossom server (`--local`, or `STLSTR_LOCAL=1`). That pinning is what `STLSTR_LOCAL_MODE` gates in `apps/stlstr/src/services/nostr.ts`: single-relay reads/publishes with no outbox routing, local Blossom fallback, plain-http URLs allowed, and read-only relay/media settings. `STLSTR_DEV_MODE` stays for dev-build concerns only (the `/napplets.dev` registry, no artifact caching).
- Use `pnpm dev --no-paja` for host + napplet build-watch only. Dev ports can be overridden with `--host`, `--app-port`, `--paja-port` or `STLSTR_DEV_HOST`, `STLSTR_APP_PORT`, `STLSTR_PAJA_PORT`, `STLSTR_NO_PAJA=1`. The local endpoints can be overridden with `VITE_STLSTR_LOCAL_RELAY` / `VITE_STLSTR_LOCAL_BLOSSOM_SERVER`.
...
- The local relay on `ws://localhost:4869` and Blossom on `http://localhost:24242` are for `pnpm local` and the tests, not for deploys. Both are assumed to be already running on this machine — tests depend on them rather than starting their own. `pnpm dev` does not use them.
...
- NAP-RESOURCE is the shell's fetch boundary: any `https` origin is grantable, but the host `fetch` enforces scheme, private-host, 10 MiB, 30s, and concurrency limits and re-types every response by sniffing so upstream `Content-Type` never reaches a napplet. `pnpm local` builds also allow `http` to private hosts for the local Blossom server.
