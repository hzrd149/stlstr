...
- Full dev is `pnpm dev`: starts `@apps/stlstr` on `127.0.0.1:5173`, builds every napplet with `vite build --watch`, writes `apps/stlstr/public/napplets.dev.json`, and starts Kehto Paja on `127.0.0.1:5197`. It talks to the **production** relays and Blossom servers, and relay/media settings are editable.
- `pnpm local` is the same environment pinned to the local relay (`--local`, or `STLSTR_LOCAL=1`). That pinning is what `STLSTR_LOCAL_MODE` gates in `apps/stlstr/src/services/nostr.ts`: single-relay reads/publishes with no outbox routing and read-only relay settings. Media server settings remain editable and direct local Blossom URLs are not blocked by policy. `STLSTR_DEV_MODE` stays for dev-build concerns only (the `/napplets.dev` registry, no artifact caching).
- Use `pnpm dev --no-paja` for host + napplet build-watch only. Dev ports can be overridden with `--host`, `--app-port`, `--paja-port` or `STLSTR_DEV_HOST`, `STLSTR_APP_PORT`, `STLSTR_PAJA_PORT`, `STLSTR_NO_PAJA=1`. The local relay endpoint can be overridden with `VITE_STLSTR_LOCAL_RELAY`.
- Scripts are namespaced `app:*` (host app, deployed as an nsyte static site) and `napplet:*` (napplets, deployed to Nostr/Blossom via the napplet CLI); `dev`, `local`, `verify`, `test`, `type-check`, and `format` stay unprefixed.
...
- The local relay on `ws://localhost:4869` is for `pnpm local` and the tests, not for deploys. It is assumed to already be running on this machine — tests depend on it rather than starting their own. `pnpm dev` does not use it.
...
- For normal dev, napplets are not run as Vite dev servers; `pnpm dev` / `pnpm local` serve their built `dist/index.html` from the host origin.
...
- NAP-RESOURCE is the shell's fetch boundary: `http`, `https`, and `blossom` origins are grantable, and the host `fetch` enforces scheme, 10 MiB, 30s, and concurrency limits and re-types every response by sniffing so upstream `Content-Type` never reaches a napplet. Local/private service URLs are allowed to be requested in every mode; if they fail, normal network/Blossom fallback behavior applies.
...
- The dev registry `apps/stlstr/public/napplets.dev.json` is generated by `pnpm dev` / `pnpm local` and is gitignored; do not commit it.
